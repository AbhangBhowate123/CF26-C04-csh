    ...
    <header className="fixed top-0 w-full z-50 flex justify-between items-center px-gutter h-16 bg-surface-container-lowest dark:bg-surface-container-lowest border-b border-outline-variant bg-opacity-70 backdrop-blur-md shadow-[0_0_15px_rgba(195,244,0,0.1)]">
      ...
      <div className="flex items-center gap-6 relative">
        ...
        <div className="flex gap-4 relative items-center">
          ...
          <div className="relative">
            <button onClick={() => toggleDropdown('bell')} className="text-on-surface-variant hover:text-primary-fixed transition-colors duration-200 relative">
              <span className="material-symbols-outlined">notifications</span>
              <span className="absolute top-0 right-0 w-2 h-2 bg-error rounded-full pulsing-red"></span>
            </button>
            {activeDropdown === 'bell' && (
              <div className="absolute right-0 mt-2 w-64 bg-[#0c0e16] border border-outline-variant rounded-md shadow-lg p-3 z-50">
                <p className="font-label-caps text-xs text-on-surface-variant mb-2">NOTIFICATIONS</p>
                <div className="border-l-2 border-error pl-2">
                  <p className="font-data-code text-sm text-error">CRITICAL: Lateral Movement</p>
                  <p className="font-data-code text-xs text-on-surface-variant">Just now</p>
                </div>
              </div>
            )}
          </div>
